...
void ota_ui_init(void)
{
    ...
    if (param_area_load(&p) == 0)
    {
        ...
        snprintf(buf, sizeof(buf), "%s @0x%08lX",
                 (p.active_slot == 0) ? "A" : "B",
                 (unsigned long)((p.active_slot == 0) ? OTA_APP_A_ADDR : OTA_APP_B_ADDR));
...
void ota_ui_refresh(void)
{
	...
	
	{
        ...
        static uint8_t  s_prev_slot = 0xFF;
        ...
        if (param_area_load(&ap) == 0)
        {
            ...
            /* 槽位 */
            if (ap.active_slot != s_prev_slot)
            {
                char sbuf[24];
                snprintf(sbuf, sizeof(sbuf), "%s @0x%08lX",
                         (ap.active_slot == 0) ? "A" : "B",
                         (unsigned long)((ap.active_slot == 0) ? OTA_APP_A_ADDR : OTA_APP_B_ADDR));
                lv_label_set_text(s_about_val[2], sbuf);
                s_prev_slot = ap.active_slot;
            }
