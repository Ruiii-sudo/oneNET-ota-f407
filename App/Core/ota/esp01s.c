...
static void esp01s_parse(void)
{
    esp01s_dma_to_ring();   /* 先把 DMA 新字节搬入接收环形缓冲 */
...
/**
 * AT 软复位：AT+RST 后等待 "ready"（老固件也可能只回 OK 或
...
int esp01s_init(void)
{
    ...
    /*  必须先启动 USART3 RX DMA 循环接收，再发 AT 命令：
       （原先的单字节中断在 flash 编程期间被 CPU stall 阻塞会 ORE
       丢字节，已整体改 DMA）DMA 硬件搬运不受 flash 编程影响。 */
    if (HAL_UART_Receive_DMA(&huart3, s_dma_rx, ESP_DMA_RX_SIZE) != HAL_OK)
    {
        /* 接收状态异常（HAL_BUSY，此前日志重试轮 init
    ...
    /* 已删除 AT+CIPDNS_DEF：实测本固件（AT 2.3.0-dev /
 ...
 * @note  接收已改 USART3 RX DMA 循环，单字节中断不再使用；
 ...
 *        esp01s_parse() 会把环形缓冲中的全部应答字节（含 AT 响应
...
/* 查询 WiFi 连接状态：AT+CWJAP?，+CWJAP:3 表示已连接并获取到 IP
...
/**
 * [FIX] 关键修复：CIPSTART 的期望串不能是 "CONNECT"——
 ...
 * [FIX] 已知主机 IP 优先直连：跳过模块 DNS，避免 DNS 挂起
...
{
    /* AT 2.3.0-dev 对已关闭连接的 CIPCLOSE 不应答或回 ERROR：
...
/* ================= 只扫描不消费的 s_data 查找 =================
 * 服务器响应（+IPD 数据）与 AT 应答（"SEND OK"/"CLOSED"）可能
...
int esp01s_send(const uint8_t *data, uint32_t len, uint32_t timeout_ms)
{
    ...
    char collect[64];
    uint32_t n = 0;
    uint8_t  error_seen = 0;   /* 已收到 ERROR 标记 */
    uint32_t err_start  = 0;   /* ERROR 到达时刻 */

    /* 丢弃上次残留应答字节；">" 提示与 "SEND OK" 同样从 s_data 匹配 */
    esp01s_flush_rx();
...
prompt_ok:
     ...
     * 只扫描 s_data 不消费，避免吞掉与 SEND OK 同批到达的 +IPD 响应。
     * AT 2.3.0-dev 开发版固件（v6/v7 监听实证）：服务器快速响应
