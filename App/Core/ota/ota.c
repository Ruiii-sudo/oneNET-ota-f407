...
static void dl_on_data(const uint8_t *data, uint32_t len, void *arg)
{
    ...
    if (c->pend_len > 0)
    {
        ...
        if (c->pend_len == 4U)
        {
            if (flash_if_write(c->backup_addr + c->offset, c->pend, 4U) != HAL_OK)
            {
                c->write_failed = 1;
                s_status.state = OTA_STATE_ERROR;
                snprintf(s_status.msg, sizeof(s_status.msg), "flash write err @%lu", (unsigned long)c->offset);
                return;
            }
            c->offset += 4U;
            c->pend_len = 0;
    ...
    {
        uint32_t n = len & ~3U;
        if (flash_if_write(c->backup_addr + c->offset, data, n) != HAL_OK)
        {
            c->write_failed = 1;
            s_status.state = OTA_STATE_ERROR;
            snprintf(s_status.msg, sizeof(s_status.msg), "flash write err @%lu", (unsigned long)c->offset);
...
static int ota_do_download(const ota_task_t *task)
{
    ...
    /* ---- 暂存分区固定为槽位 B：App 固定链接/运行在槽位 A，
       BootLoader 在 TRY_NEW 启动时把 B 校验后拷贝到 A ---- */
    c.backup_addr = OTA_APP_B_ADDR;
    c.tid         = task->tid;
    c.total       = task->size;
    c.write_failed = 0;
    c.pend_len    = 0;
    ...
    if (!resume_ok)
    {
        ...
        {
            snprintf(s_status.msg, sizeof(s_status.msg), "erase fail");
            s_status.state = OTA_STATE_ERROR;
            return -1;
        }
        c.offset = 0;
    ...
    if (c.pend_len > 0)
    {
        ...
        if (flash_if_write(c.backup_addr + c.offset, tail, 4U) != HAL_OK)
        {
            snprintf(s_status.msg, sizeof(s_status.msg), "flash write err @%lu", (unsigned long)c.offset);
            s_status.state = OTA_STATE_ERROR;
            return -1;
...
/* 校验暂存槽（B）镜像 SHA-256 与参数区记录一致（用于跳过重复下载） */
static int ota_verify_backup_sha(const ota_param_t *p)
{
    uint8_t digest[SHA256_DIGEST_SIZE];
    uint32_t addr = OTA_APP_B_ADDR;
    ...
    while (remaining > 0)
    {
        ...
        flash_if_read(addr + (p->image_len - remaining), chunk, take);
